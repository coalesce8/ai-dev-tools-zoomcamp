...
Status: v1 starter. This document defines the protocol and behavior implemented by Agent Relay, independent of storage backend.

The starter runs against SQLite by default so it needs no external service, and also supports PostgreSQL (see `compose.yaml`) for a real deployment. The HTTP protocol, credential rules, task lifecycle, and delivery guarantees are identical on both backends.
...
- **Storage (SQLite or PostgreSQL):** persists agents, tasks, and delivery attempts. On SQLite, WAL mode and a `BEGIN IMMEDIATE` writer transaction coordinate concurrent claims across API/worker processes. On PostgreSQL, the same operations use ordinary transactions with row locking (`FOR UPDATE`, and `FOR UPDATE SKIP LOCKED` for claims) instead, without changing the protocol.
...
Claiming atomically changes the task to `processing` and creates a delivery attempt with a new claim token. Concurrent workers cannot successfully claim the same task while that lease is active. On SQLite this is a `BEGIN IMMEDIATE` transaction (SQLite has no `FOR UPDATE SKIP LOCKED`); on PostgreSQL it is a transaction with `FOR UPDATE SKIP LOCKED` row locking, so unrelated queued tasks can still be claimed concurrently.
