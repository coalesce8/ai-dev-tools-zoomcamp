...
writer transaction (:func:`database.immediate_transaction`) and lock the
specific rows they change with :func:`database.for_update`, which is a no-op
on SQLite (serialized by that transaction already) and real row locking on
PostgreSQL.
"""
...
def create_task(sender_id: str, recipient_id: str, input_text: str, idempotency_key: str | None) -> dict[str, str]:
    # On SQLite, BEGIN IMMEDIATE already serializes this whole check-then-insert
    # against other writers. On PostgreSQL two API processes can still race
    # between the idempotency lookup and the insert; the unique constraint on
    # (sender_id, idempotency_key) catches that, and we resolve it below by
    # reading back the row the other transaction committed.
    try:
        with immediate_transaction() as db:
        ...
        if idempotency_key is None:
            raise
        with db_session() as db:
            return _existing_task_for_idempotency_key(db, sender_id, recipient_id, input_text, idempotency_key)
