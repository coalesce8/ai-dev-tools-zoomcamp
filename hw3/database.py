...
pragmas and the writer transaction used to make claims/heartbeats/terminal
submissions/recovery race-safe. It supports two backends, selected by
``RELAY_DATABASE_URL``/``DATABASE_URL``:

- **SQLite** (default, zero external services): a ``BEGIN IMMEDIATE`` writer
  reservation serializes the relevant transactions, since SQLite has no
  ``FOR UPDATE SKIP LOCKED``.
- **PostgreSQL**: ordinary transactions plus explicit row locking
  (``FOR UPDATE`` / ``FOR UPDATE SKIP LOCKED``, applied in :mod:`storage`)
  serialize the same operations. See ``compose.yaml`` for a Postgres-backed
  deployment.
...
def immediate_transaction() -> Generator[Session, None, None]:
    ...
    SQLite has no ``FOR UPDATE SKIP LOCKED``. On PostgreSQL, connecting
    already opens a transaction; callers instead lock the specific rows they
    are about to change with :func:`for_update`, which lets unrelated tasks
    claim concurrently instead of serializing on one global writer.
    """
...
def for_update(query: Any, *, skip_locked: bool = False) -> Any:
    """Row-lock ``query`` for the PostgreSQL claim/heartbeat/terminal/recovery paths.

    SQLite ignores ``FOR UPDATE`` clauses entirely (its ``BEGIN IMMEDIATE``
    writer transaction already serializes these operations), so this is a
    no-op there and only takes effect once ``RELAY_DATABASE_URL`` points at
    PostgreSQL.
    """
